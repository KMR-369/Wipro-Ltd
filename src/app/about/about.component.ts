...
export class AboutComponent implements OnInit {
  ...
  items =["mouse","keyboard","Scanner","Printer","Speakers"];
  qty = [50,35,10,100,75];
  ...
  title:string = "Top 5 Movies of HollyWood";
   ...
   // Nested Array - Employee Array has nested skills Array
   employees = [
    {
  ...
  getColor() { // === strict equality comparison operator
               // for the values which are not of a similar type
    return this.serverStatus === 'Online' ? 'green' : 'red';
  }

  getColor1(country:string):string { (2)
    switch (country) {
      case 'UK':
        return 'green';
      case 'USA':
